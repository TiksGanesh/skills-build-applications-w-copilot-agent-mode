...
const Teams = () => {
  ...
  const load = () => {
    ...
    fetch(url)
      ...
      .catch(err => { console.error('Teams fetch error:', err); setError('Failed to load teams'); })
  ...
  return (
    <div className="card shadow-sm">
      ...
      <div className="card-body">
        <form className="row g-2 mb-3" onSubmit={e => e.preventDefault()}>
          <div className="col-auto">
            <input className="form-control" placeholder="Search teams..." value={query} onChange={e => setQuery(e.target.value)} />
          </div>
        </form>
        ...
        <div className="table-responsive">
          <table className="table table-striped table-hover align-middle">
            ...
            <tbody>
              {filtered.map(t => (
                <tr key={t.id || t._id || t.name}>
                  <td>{t.name}</td>
                  <td>
                    <button type="button" className="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#teamsModal" onClick={() => setSelected(t)}>View</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="modal fade" id="teamsModal" tabIndex="-1" aria-hidden="true">
