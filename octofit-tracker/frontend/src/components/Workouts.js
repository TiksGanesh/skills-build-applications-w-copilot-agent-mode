...
const Workouts = () => {
  ...
  const load = () => {
    ...
    fetch(url)
      ...
      .catch(err => { console.error('Workouts fetch error:', err); setError('Failed to load workouts'); })
  ...
  return (
    <div className="card shadow-sm">
      ...
      <div className="card-body">
        <form className="row g-2 mb-3" onSubmit={e => e.preventDefault()}>
          <div className="col-auto">
            <input className="form-control" placeholder="Search workouts..." value={query} onChange={e => setQuery(e.target.value)} />
          </div>
        </form>
            ...
            <tbody>
              {filtered.map((w, idx) => (
                <tr key={w.id || w._id || idx}>
                  <td>{w.name}</td>
                  <td>{w.duration}</td>
                  <td className="text-truncate" style={{maxWidth: '420px'}}>{w.description}</td>
                  <td>
                    <button type="button" className="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#workoutsModal" onClick={() => setSelected(w)}>View</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="modal fade" id="workoutsModal" tabIndex="-1" aria-hidden="true">
